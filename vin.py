...
def lexical_analyzer(text, file_path):
    token_specification = [
        ('KEYWORD', r'\b(const|val|Int)\b', 14),
        ('IDENTIFIER', r'\b[a-zA-Z_][a-zA-Z_0-9]*\b', 2),
        ('NUMBER', r'\b[+-]?\d+\b', 1), 
        ('COLON', r':', 10),
        ('EQUALS', r'=', 10),
        ('SEMICOLON', r';', 16),
    ...
    for line in text.splitlines():
        ...
        while pos < len(line):
            ...
            if match:
                ...
                if type_ == 'MISMATCH':
                    output_table.insert("", "end", values=(99, 'Ошибка', value, f'с {start} по {end} символ', file_path.split('/')[-1], line_number))
                elif token_specification:
                    for token in token_specification:
                        if token[0] == type_:
                            type_code, type_desc = token[2], type_[1:]
                            if type_code:
                                output_table.insert("", "end", values=(type_code, type_desc, value, f'с {start} по {end} символ', file_path.split('/')[-1], line_number))
            else:
                break
        line_number += 1
...
def syntax_analysis():
    ...
    if current_tab and hasattr(current_tab, "text_area"):
        ...
        file_path = current_tab.file_path if hasattr(current_tab, "file_path") else "Без имени"
...
icons = {
    ...
    "пуск": PhotoImage(file="C:/Users/Никита/Desktop/КР/icons/pusk.png"),
}
...
output_table.column("type", width=150)
