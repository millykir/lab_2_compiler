...
def scan_code(text, file_path=None):
    i = 0
    length = len(text)
    tokens = []
    ...
    while i < length:
        ...
        if char.isspace():
            tokens.append((token_types["SEPARATOR"], "разделитель", char, i+1, i+1))
            i += 1
            continue

        # Числа, включая знаки "+" и "-"
        if char.isdigit() or char in "+-":
            start = i
            # Обработка знака перед числом
            if char in "+-":
                i += 1
                char = text[i]  # следующий символ после знака
            while i < length and (text[i].isdigit() or text[i] == '.'):
                i += 1
            tokens.append((token_types["NUMBER"], "число", text[start:i], start+1, i))
            continue
        ...
        if char.isalpha() or char == "_":
            ...
            tokens.append((token_types[token_type], "ключевое слово" if token_type == "KEYWORD" else "идентификатор", word, start+1, i))
            continue
        ...
        if char == '"' or char == "'":
            ...
            start = i
            i += 1
            while i < length and text[i] != quote_char:
                i += 1
            i += 1  # закрывающая кавычка
            tokens.append((token_types["STRING"], "заданый формат", text[start:i], start+1, i))
            continue

        # Операторы
        if char == '=':
            tokens.append((token_types["ASSIGN"], "оператор присваивания", char, i+1, i+1))
            i += 1
            continue

        # Конец команды
        if char == ';':
            tokens.append((token_types["END"], "конец оператора", char, i+1, i+1))
            i += 1
            continue

        # Скобки и точка
        if char == '.':
            tokens.append((token_types["DOT"], "точка", char, i+1, i+1))
            i += 1
            continue
        if char == '(':
            tokens.append((token_types["LEFT_PAREN"], "левая скобка", char, i+1, i+1))
            i += 1
            continue
        if char == ')':
            tokens.append((token_types["RIGHT_PAREN"], "правая скобка", char, i+1, i+1))
            i += 1
            continue

        # Всё остальное — ошибка
        tokens.append((token_types["UNKNOWN"], "недопустимый символ", char, i+1, i+1))
        i += 1
    ...
    for token in tokens:
        code, typ, lexeme, start_pos, end_pos = token
        output_table.insert("", "end", values=(
            code, typ, lexeme, f"{start_pos}-{end_pos}", file_path or "–"
        ))
